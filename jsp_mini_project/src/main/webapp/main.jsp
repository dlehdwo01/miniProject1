			...
			<div id="section">
				<div id="section-contents">
					...
					<table>
						...
						<%
						String sql = "select rownum, t.* from (SELECT cus_name, TO_CHAR(cus_birth,'yy/mm/dd') as cus_birth,cus_phone,cus_addr ,product_no,product_pkno ,TO_CHAR(sell_date,'YY/MM/DD') as sell_date,SALES_NAME,STATUS FROM djl_cus_info c LEFT JOIN djl_sell s ON c.cus_no = s.cus_no) t";
						ResultSet srs = stmt.executeQuery(sql);

						while (srs.next()) {
						%>
						<tr>
							<td><%=srs.getString("rownum")%></td>
							<td><%=srs.getString("cus_name")%></td>
							<td><%=srs.getString("cus_birth")%></td>
							<td><%=srs.getString("cus_phone")%></td>
							<td style="text-align: left;"><%=srs.getString("cus_addr")%></td>
							<td><%=srs.getString("product_no")%></td>
							<td><%=srs.getString("product_pkno")%></td>
							<td><%=srs.getString("sell_date")%></td>
							<td><%=srs.getString("sales_name")%></td>
							<td><%=srs.getString("status")%></td>
							<td>
								<input type="checkbox">
							</td>
						</tr>
